...
- **Kind-aware routing.** `resolvePreviewFor(preview, ext)` routes on the discriminated `preview.kind` first: `image` → the extension-registered component or ImagePreview when unregistered; `empty` → BinaryPreview (status, never overridden); non-text (`binary`/`too-large`) → the extension-registered component or BinaryPreview when unregistered; `text` → the extension-registered component or TextPreview when unregistered (e.g. an extension-less file). Add a `FilePreview` variant by extending the union in `src/protocol.ts`, producing it in `src/index.ts`, and handling its `kind` in `resolvePreviewFor` plus the owning preview component.
