...
export const FileTree = forwardRef(function FileTree({ sessionId, helpers, fetchList, t, autoRefresh }, ref) {
    ...
    return (_jsxs("div", { className: "dsh-fe-tree", children: [_jsx(VirtualList, { rowCount: flat.length, rowHeight: TREE_ROW_HEIGHT, rowKey: (i) => flat[i].path, renderRow: (i) => (_jsx(TreeRow, { entry: flat[i].entry, depth: flat[i].depth, expanded: expanded, onDisclosureClick: handleDisclosureClick, helpers: helpers, onOpenMenu: openMenu })) }), _jsx(FileContextMenu, { open: menu.open, anchor: menu.anchor, items: menuItems, onClose: closeMenu })] }));
});
