...
import { cp as fsCp, mkdir as fsMkdir, open, readFile, readdir, realpath, rename as fsRename, rm as fsRm, stat, writeFile, } from 'node:fs/promises';
import { basename, dirname, extname, join, relative, resolve, sep } from 'node:path';
import { COPY_ACTION, CREATE_FILE_ACTION, DELETE_ACTION, FILE_EXPLORER_ROUTE, MKDIR_ACTION, MOVE_ACTION, PDF_ACTION, RENAME_ACTION, STATIC_FILES_ROUTE, } from "./protocol.js";
// ---------------------------------------------------------------------------
...
async function list(root, input, showHidden = false) {
    ...
    const entries = await Promise.all(children
        ...
        if (child.isDirectory()) {
            return { name: child.name, path: childPath, kind: 'directory', mtimeMs: info.mtimeMs };
        }
        return { name: child.name, path: childPath, kind: 'file', size: info.size, mtimeMs: info.mtimeMs };
    }));
...
function assertNotRoot(input) {
    if (input === '' || input === '.')
        throw new Error('cannot operate on the workspace root');
}
        ...
        const disposeApiRoute = ctx.webServer.register({
            ...
            handler: async (req, res) => {
                try {
                    ...
                    if (action === CREATE_FILE_ACTION || action === MKDIR_ACTION || action === RENAME_ACTION ||
