...
export function FileContextMenu({ open, getAnchorRect, items, onClose }) {
    ...
    if (!open)
        return null;
    const menu = (_jsx("div", { ref: menuRef, className: "dsh-fe-menu", "data-fe-menu": true, role: "menu", style: { position: 'fixed', ...(fixedPos ?? MEASURE_STYLE) }, children: items.map((item) => (_jsxs("div", { className: 'dsh-fe-menu-item' + (item.danger ? ' dsh-fe-menu-item--danger' : ''), "data-fe-menu-item": item.id, role: "menuitem", onClick: () => {
                item.onSelect();
                onClose();
