...
DSH Web 的侧边栏文件浏览器。在左侧边栏顶部注入「会话 / 文件」Tab，点「文件」在当前工作区内浏览文件；点文件在右侧浮出可拖拽/缩放的预览框。点击会话区"生成的文件"芯片或工具行文件链接，会自动在预览框中打开对应文件，而非跳到系统默认应用。

## 功能

1. **侧边栏入口**：侧边栏顶部「会话 / 文件」Tab，点「文件」显示文件树，点「会话」切回会话列表。
2. **文件浏览**：懒加载目录树，跟随当前会话的工作区根目录，切换会话时自动刷新。
3. **悬浮预览框**：点文件在右侧浮出可拖拽/缩放/最小化/关闭的预览框。
4. **文件预览**：内置文本（源码）、Markdown（渲染 + 源码切换）、图片（data URL）、二进制（文件信息）预览。
5. **可扩展预览注册表**：`registerPreview(ext, component)` 按文件扩展名注册预览组件，未注册的扩展名回退到 `binary` 预览。新增蛋白质结构（`.cif`/`.pdb` → Mol*）、CSV、PDF 等预览器无需改动面板本身。
6. **右键菜单**：打开 / 复制路径 / 复制相对路径。
7. **快捷键**：`Ctrl/Cmd+Shift+E` 在「会话 / 文件」间切换。
...
- **侧边栏注入为 best-effort**：通过 MutationObserver 查找会话列表的 `role="tree"` 锚点来注入「会话 / 文件」Tab，DSH 侧边栏 DOM 结构变化时可能失效（找不到锚点时插件静默降级，不破坏 DSH）。注入逻辑隔离在 `src/client/mount-sidebar.ts`，改结构只需改这一处。
