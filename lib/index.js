...
async function servePdf(root, input, res) {
	const target = await inside(root, input);
	if (!(await stat(target.absolute)).isFile()) throw new Error("path is not a file");
		...
		const disposeRoute = ctx.webServer.register({
			...
			handler: async (req, res) => {
				try {
					...
					if (action === "pdf") return await servePdf(root, path, res);
					if (action === "raw") {
						...
						if (rangeHeader) {
							...
							if (m) {
								...
								const end = m[2] ? Number(m[2]) : void 0;
								range = end !== void 0 ? {
									offset: start,
									limit: end - start + 1
								} : { offset: start };
...
//#endregion
export { apply, capBytes, inject, inside, list, preview, raw, write };
